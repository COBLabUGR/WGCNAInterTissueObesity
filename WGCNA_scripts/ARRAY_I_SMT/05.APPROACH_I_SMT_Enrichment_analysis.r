...
purple <- read.csv2("/media/mireia/Disco Duro HDD/Documentos/R/ANALISIS_GO/MODULE_GENE_ID_ALL_MUSCULO/purple.csv", header=TRUE, sep=";", stringsAsFactors=F, dec=",", na.strings=c(""," ","NaN","NA"),row.names=1)
ae.annots <- AnnotationDbi::select(x= hgu133plus2.db, keys=purple$Code,  columns = c("ENTREZID","SYMBOL"),  keytype="PROBEID")
ae.annots
...
write.csv2(MERGED_, file="/media/mireia/Disco Duro HDD/Documentos/R/ANALISIS_GO/ENRICHMENT_RESULTS/ALL_MUSCLE/ae_annot_simplificado_/ae_annot_simplificado_purple.csv", row.names=TRUE)

write.csv2(TO_SAVES, file="/media/mireia/Disco Duro HDD/Documentos/R/ANALISIS_GO/ENRICHMENT_RESULTS/ALL_MUSCLE/enrichmentGO/purple_enrichmentGO.csv", row.names=TRUE)

write.csv2(TO_SAVES, file="/media/mireia/Disco Duro HDD/Documentos/R/ANALISIS_GO/ENRICHMENT_RESULTS/ALL_MUSCLE/enrichmentKEGG/purple_enrichmentKEGG.csv", row.names=TRUE)
